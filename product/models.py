...
class Item(models.Model):
    id = ShortUUIDField(primary_key=True,editable=False, default=shortuuid.uuid)
    name=models.CharField(max_length=50)
...
class Offer(models.Model):
    ...
    @property
    def price(self):
        new_price = self.item.public_price - ((self.percentage/100)*self.item.public_price)
        return new_price
#
class Idle(models.Model):
    ...
    @property
    def price(self):
        new_price = self.product.public_price - ((self.percentage/100)*self.product.public_price)
        return new_price
#
